...
function filterIt(flights_jan_01_2008, searchKey) {
  return flights_jan_01_2008.filter(function (obj) {
    if (obj != null) {
...
// fd = filterIt(flights_jan_01_2008, "ual");

var count = 0;
function findNumberOfhrs(fd1, k) {
  ...
  for (var i = 0; i < fd1.length; i++) {
    for (key in fd1[i]) {
      if (fd1[i][key].indexOf(k) != -1) {
        // results.push(fd1[i].time);
        var sw = fd1[i]?.time;
        var subS = sw?.substr(0, 2);

        if (subS=== k) {
          count = count + 1;
        }

        // console.log(sw.substr(0, 2));
      }
    }
  }
}
// findNumberOfhrs(fd, "14");
// console.log("find 'x'", count);
...
var name ="vishnu"
var dataAirlines =[];
var dataAirlinesKeys =[];

for (const property in airlines) {
//   console.log(`${property}: ${airlines[property]}`);
  dataAirlines.push(airlines[property])
  dataAirlinesKeys.push(property)

}
console.log("frt",dataAirlinesKeys)
  for (var i = -1; i < 12; i++) {
    ...
    tableBody.appendChild(tr);
            ...
            var dAk = dataAirlinesKeys[i];
            var dAkValue = dataAirlines[i];
            fd = filterIt(flights_jan_01_2008, dAk);
            // console.log(fd)
             ...
             for (var j = -1; j <= 24; j++) {
                 ...
                 if(j==-1){
                      var td = document.createElement('TD');
                        td.width = '75';
                            td?.appendChild(document.createTextNode(dAkValue));
                            tr?.appendChild(td);
                 }
                 else if(i==-1){
        ...
        if(j==24){
            ...
            tr.appendChild(td);
                            td.setAttribute("style", "background-color:red; font-size:2em; ");
